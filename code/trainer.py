...
def save_img_results(imgs_tcpu, fake_imgs, num_imgs,
    ...
    Image.fromarray(np.uint8(real_img_set))
...
class condGANTrainer(object):
    ...
    def prepare_data(self, data):
        imgs, w_imgs, t_lable, _ = data

        real_vimgs, wrong_vimgs = [], []
        if cfg.CUDA:
            vlable = Variable(t_lable).cuda()
        else:
            vlable = Variable(t_lable)
        ...
        return imgs, real_vimgs, wrong_vimgs, vlable
    ...
    def train_Dnet(self, idx, count):
        ...
        #
        # print('real_logits',real_logits[0])
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for step, data in enumerate(self.data_loader, 0):
                ...
                self.imgs_tcpu, self.real_imgs, self.wrong_imgs, \
                    self.txt_lable = self.prepare_data(data)
                ...
                #     self.netG(noise, self.txt_lable)
                self.fake_imgs, self.mu = \
                    self.netG(noise, self.txt_lable)
                # print('mu,',self.mu)
                # print(self.mu.shape)
                ...
                if count % cfg.TRAIN.SNAPSHOT_INTERVAL == 0:
                    ...
                    #
                    # self.fake_imgs, _, _ = \
                    #     self.netG(fixed_noise, self.txt_lable)
                    self.fake_imgs, _ = \
                        self.netG(fixed_noise, self.txt_lable)
                    ...
                    if len(predictions) > 500:
                        ...
                        m_nlpp = summary.scalar('NLPP_mean', mean_nlpp) # 负对数后验概率
    ...
    def save_singleimages(self, images, filenames,
        ...
        for i in range(images.size(0)):
            ...
            fullpath = '%s_%d.png' % (s_tmp, imsize)
            # range from [-1, 1] to [0, 255]
            img = images[i].add(1).div(2).mul(255).clamp(0, 255).byte()
            ndarr = img.permute(1, 2, 0).data.cpu().numpy()
            im = Image.fromarray(ndarr)
            im.save(fullpath)
            # print('images saved:' + fullpath)
    ...
    def evaluate(self, split_dir):
        if cfg.TRAIN.NET_G == '':
            ...
            for step, data in enumerate(self.data_loader, 0):
                ...
                fake_img_list = []
                fake_imgs, _ = netG(noise, t_embeddings[:, :])
