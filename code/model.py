...
class CA_NET(nn.Module):
    ...
    def forward(self, text_embedding):
        ...
        c_code = self.reparametrize(mu, logvar)# 从均值/方差中采样得到c_code
        # # lable为标签列表
        # self.lable =
        # # 每次采样一个batch size
        # c_code = self.lable(:,self.batch_size)
...
class INIT_STAGE_G(nn.Module):
    ...
    def forward(self, z_code, c_code=None):
        if cfg.GAN.B_CONDITION and c_code is not None:
            ...
            # print (c_code.shape)
            # print (z_code.shape)
            in_code = torch.cat((c_code, z_code), 1) # with noise
            # in_code = c_code  # no noise
        else:
            in_code = z_code
